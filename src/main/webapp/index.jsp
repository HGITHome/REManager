...
<head>
    ...
    <title>Admin</title>
    <script>
        var userNameTmp = '<%=session.getAttribute("name")%>';
        var userIdTmp = <%=session.getAttribute("id")%>;
        var userPicTmp = '<%=session.getAttribute("miniUrl")%>';
        var userEmailTmp = '<%=session.getAttribute("email")%>';
        var userAddrTmp = '<%=session.getAttribute("address")%>';
        var userBirthDayTmp = '<%=session.getAttribute("birthday")%>';
        var userQQTmp = '<%=session.getAttribute("QQ")%>';
        var userPhoneTmp = '<%=session.getAttribute("phone")%>';
        var userEduTmp = '<%=session.getAttribute("education")%>';
        var userEntryTimeTmp = '<%=session.getAttribute("entry_time")%>';
        function getEduType () {
            switch (userEduTmp){
                case 'PRIMARY':
                    return '小学';
		...
		var role ='<%=session.getAttribute("roles")%>';
		function isHasAuthority(parameter){
			if(window.role == "ROLE_SUPER"){
				return true;
			};
            for(var i in parameter){
                if(window.role == parameter[i]) return true;
            }
            return false;
		} 
		</script>
    ...
    <script id="microloader" data-app="4348c289-1075-4d3a-9bc6-3c3dd7e35a49" type="text/javascript">var Ext = Ext || { }; Ext.manifest = Ext.manifest || "modern.json";// @tag core
// @define Ext.Boot
